import { ANIM_DEFS } from './constants.js';
import { gs, clones } from '../state.js';

const SPRITE_DATA = {
"idle": "data:image/png;base64,",
"sleep": "data:image/png;base64,",
"eat": "data:image/png;base64,",
"happy": "data:image/png;base64,",
"sad": "data:image/png;base64,"
};

export const spriteImgs = {};
for (const [k, src] of Object.entries(SPRITE_DATA)) {
  const img = new Image(); img.src = src; spriteImgs[k] = img;
}

const canvas = document.getElementById('clawd-canvas');
export const ctx = canvas.getContext('2d');
ctx.imageSmoothingEnabled = false;

// Visual parameters per growth stage: [scale, extra filter prefix]
const STAGE_VIS = [
  [0.42, ''],                                         // 0: 蛋
  [0.54, ''],                                         // 1: 婴儿
  [0.68, ''],                                         // 2: 幼儿
  [0.82, ''],                                         // 3: 少年
  [1.00, ''],                                         // 4: 成年
  [1.00, 'sepia(22%) brightness(0.82) contrast(0.9) '], // 5: 老年
];

function _drawEgg(ectx, frame) {
  const wobble = [0, 0.06, 0, -0.06][frame % 4];
  ectx.clearRect(0, 0, 16, 16);
  ectx.save();
  ectx.translate(8, 15);   // pivot at bottom-center of egg
  ectx.rotate(wobble);
  ectx.translate(-8, -15);

  // Outer border
  ectx.fillStyle = '#b89a50';
  ectx.beginPath(); ectx.ellipse(8, 9, 5.5, 7, 0, 0, Math.PI*2); ectx.fill();

  // Main shell — cream
  ectx.fillStyle = '#f2e4b4';
  ectx.beginPath(); ectx.ellipse(7.8, 8.8, 4.6, 6.2, 0, 0, Math.PI*2); ectx.fill();

  // Right/bottom shadow
  ectx.fillStyle = 'rgba(160,110,30,0.22)';
  ectx.beginPath(); ectx.ellipse(9.5, 11, 3, 3.5, 0.3, 0, Math.PI*2); ectx.fill();

  // Top-left highlight
  ectx.fillStyle = 'rgba(255,255,255,0.60)';
  ectx.beginPath(); ectx.ellipse(5.8, 6.2, 1.8, 2.6, -0.3, 0, Math.PI*2); ectx.fill();

  // Crack — visible in the last 1/3 of the egg stage (gameHour ≥ 20 on day 0)
  if (gs.day === 0 && gs.gameHour >= 20) {
    ectx.strokeStyle = '#7a5020';
    ectx.lineWidth = 0.8;
    ectx.beginPath();
    ectx.moveTo(8, 4.5); ectx.lineTo(9.5, 7); ectx.lineTo(7.5, 9); ectx.lineTo(9, 11);
    ectx.stroke();
  }
  ectx.restore();
}

export function drawEntityFrame(ectx, anim, frame, flip, dirty=0) {
  ectx.clearRect(0, 0, 16, 16);

  if (gs.stage === 0) {
    _drawEgg(ectx, frame);
    return; // eggs don't get dirty
  }

  const img = spriteImgs[anim];
  if (!img || !img.complete) return;
  ectx.save();
  if (flip) { ectx.translate(16, 0); ectx.scale(-1, 1); }
  ectx.drawImage(img, frame*16, 0, 16, 16, 0, 0, 16, 16);
  if (dirty > 5) {
    const spots = [[2,8],[5,12],[9,6],[12,10],[7,14],[3,4],[11,3],[8,11],[14,7],[4,14],[6,5],[10,13]];
    const cnt = Math.min(spots.length, Math.ceil(dirty/8));
    ectx.globalAlpha = Math.min(0.8, dirty/100*0.9);
    ectx.fillStyle = '#6b3a1a';
    for (let i=0; i<cnt; i++) ectx.fillRect(spots[i][0], spots[i][1], 1, 1);
    ectx.globalAlpha = 1;
  }
  ectx.restore();
}

export function drawFrame() {
  drawEntityFrame(ctx, gs.currentAnim, gs.animFrame, gs.flipX, gs.entityDirty);
}

export function tickAnim(dt) {
  const def = ANIM_DEFS[gs.currentAnim];
  gs.animTimer += dt;
  if (gs.animTimer >= 1/def.fps) {
    gs.animTimer = 0;
    gs.animFrame = (gs.animFrame + 1) % def.frames;
    drawFrame();
  }
}

export function setAnim(name, reset=true) {
  if (gs.currentAnim === name && !reset) return;
  gs.currentAnim = name;
  if (reset) { gs.animFrame = 0; gs.animTimer = 0; }
  drawFrame();
}

// Apply scale + filter to a canvas element for the given stage index
function _styleCanvas(cnv, stageIdx) {
  const [scale, extra] = STAGE_VIS[stageIdx] || STAGE_VIS[4];
  cnv.style.transformOrigin = 'bottom center';
  cnv.style.transform       = scale < 1 ? `scale(${scale})` : '';
  cnv.style.transition      = 'transform 0.7s ease-out';
  cnv.style.filter          = `${extra}drop-shadow(0 4px 0 rgba(0,0,0,.4))`;
}

// Call whenever gs.stage changes (and on init)
export function applyStageVisuals() {
  _styleCanvas(document.getElementById('clawd-canvas'), gs.stage);
  clones.forEach(c => _styleCanvas(c.ctx.canvas, gs.stage));
}
